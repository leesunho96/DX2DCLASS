#include "stdafx.h"
#include "AlphaVlendSprite.h"

//-----------------------------------------------------------------------------
//AlphaBlendSprite
//-----------------------------------------------------------------------------

AlphaBlendSprite::AlphaBlendSprite(wstring textureFile, wstring shaderFile)
{
...
void AlphaBlendSprite::Initialize(wstring spriteFile, wstring shaderFile, float startX, float startY, float endX, float endY)
{
	...
	srv = AlphaBlendSprites::Load(spriteFile);
...
AlphaBlendSprite::~AlphaBlendSprite()
{
	...
	AlphaBlendSprites::Remove(textureFile);
}
...
void AlphaBlendSprite::Update(D3DXMATRIX& V, D3DXMATRIX& P)
{
	...
	world = S * R * T;
	D3DXMATRIX alphablend;
	alphablend._11 = fAlphaBlendValues;
	shader->AsMatrix("World")->SetMatrix(world);
	shader->AsMatrix("fAlphaBlendValues")->SetMatrix(alphablend);
...
void AlphaBlendSprite::CreateBound()
{
...
void AlphaBlendSprite::Position(float x, float y)
{
	Position(D3DXVECTOR2(x, y));
}

void AlphaBlendSprite::Position(D3DXVECTOR2 & vec)
{
	position = vec;
}

void AlphaBlendSprite::Scale(float x, float y)
{
	Scale(D3DXVECTOR2(x, y));
}

void AlphaBlendSprite::Scale(D3DXVECTOR2 & vec)
{
	scale = vec;
}

void AlphaBlendSprite::SetAbsoluteScale(float x, float y)
{
	SetAbsoluteScale(D3DXVECTOR2(x, y));
}

void AlphaBlendSprite::SetAbsoluteScale(D3DXVECTOR2 & vec)
{
	scale.x = scale.x / textureSize.x * vec.x;
	scale.y = scale.y / textureSize.y * vec.y;
}

void AlphaBlendSprite::Rotation(float x, float y, float z)
{
	Rotation(D3DXVECTOR3(x, y, z));
}

void AlphaBlendSprite::Rotation(D3DXVECTOR3 & vec)
{
	rotation = vec;

}

void AlphaBlendSprite::RotationDegree(float x, float y, float z)
{
	RotationDegree(D3DXVECTOR3(x, y, z));
}

void AlphaBlendSprite::RotationDegree(D3DXVECTOR3 & vec)
{
...
D3DXVECTOR3 AlphaBlendSprite::RotationDegree()
{
	D3DXVECTOR3 vec;
...
bool AlphaBlendSprite::AABB(D3DXVECTOR2 position)
{

	return AABB(this, position);
}

bool AlphaBlendSprite::AABB(AlphaBlendSprite * b)
{
...
// 
bool AlphaBlendSprite::OBB(AlphaBlendSprite * b)
{
	return OBB(this, b);
}

bool AlphaBlendSprite::AABB(AlphaBlendSprite * a, D3DXVECTOR2 & position)
{
	...
	// position이 AlphaBlendSprite와 겹치는 영역이 있는지 검사 => AlphaBlendSprite 내부에 있는지 검사함
	bool b = true;
	b &= position.x > left;
	b &= position.x <= right;
	b &= position.y > bottom;
...
bool AlphaBlendSprite::AABB(AlphaBlendSprite * a, AlphaBlendSprite * b)
{
	float xScale = a->scale.x * a->TextureSize().x * 0.5f;
	float yScale = a->scale.y* a->TextureSize().y * 0.5f;
...
bool AlphaBlendSprite::OBB(AlphaBlendSprite * a, AlphaBlendSprite * b)
{
	OBBDesc obbA, obbB;
	//float xScale, yScale;
...
void AlphaBlendSprite::CreateOBB(OUT OBBDesc * out, D3DXVECTOR2 & position, D3DXMATRIX & world, D3DXVECTOR2 & length)
{
...
float AlphaBlendSprite::SeperateAxis(D3DXVECTOR2 seperate, D3DXVECTOR2 & e1, D3DXVECTOR2 & e2)
{
...
bool AlphaBlendSprite::CheckOBB(OBBDesc & obbA, OBBDesc & obbB)
{
...
//AlphaBlendSprites
//-----------------------------------------------------------------------------
map<wstring, AlphaBlendSprites::SpriteDesc> AlphaBlendSprites::spriteMap;

ID3D11ShaderResourceView * AlphaBlendSprites::Load(wstring file)
{
...
void AlphaBlendSprites::Remove(wstring file)
{
